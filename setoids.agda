...
{- Setoids à la Pierre-Marie Pédrot. A setoid is the data of
   * a type of elements [s-el]
   * a binary relation [s-rel]
   * for any element [x], a predicate [s-refl] on [s-rel x x] which carves out reflexive proofs -}

{- Note that we do not assume that [s-rel] is transitive or symmetric.
   This will only be the case for *fibrant* setoids, which are defined later on. -}
...
{- A point of a setoid is an element [x] along with a reflexive proof of [s-rel x x] -}
...
{- Morphisms of setoids.
   Our morphisms preserve reflexive proofs. This is essential to get ξ/η later on. -}
...
{- Sections of dependent setoids (i.e. dependent functions)
   This is a straightforward generalisation of [SetoidMorphism] to dependent setoids. -}
...
{- Relations on a setoid (used for quotients)
   An element of [SetoidRelation A] should be the same as a setoid morphism [A × A → Ω] -}
...
{- Now that we have defined the category of setoids (although we have not proved that it is a
   category yet), we define some examples of setoids : natural numbers, dependent products, dependent sums -}
...
{- Classifier of propositions
   * option 1 : all equalities are reflexive
   * option 2 : only the identity is reflexive
   I am not sure which one works best. -}

SetoidΩ : (ℓ : Level) → Setoid (lsuc ℓ)
SetoidΩ ℓ .s-el = Set ℓ
SetoidΩ ℓ .s-rel x y = Lift₁ (x ↔ y)
SetoidΩ ℓ .s-refl x e = ⊤ℓ

{- Quotient of a setoid by a relation. We need some auxiliary definitions -}
...
SetoidQ : {ℓ : Level} (A : Setoid ℓ) (R : SetoidRelation A) → Setoid ℓ
SetoidQ A R .s-el = SetoidPt A
SetoidQ A R .s-rel a b = quoEq A A (SetoidEq {X = A}) (R .r-el) (R .r-el) a b
SetoidQ A R .s-refl a = quoEq-refl A (λ a e → A .s-refl (a .p-el) e) (R .r-el) a

{- Universe of small setoids. This universe is inductively generated by
   * the setoid of natural numbers
   * dependent products
   * dependent sums
   * quotients
   * embeddings from Ω -}
   
{- In order to avoid induction-recursion, we do this definition in several steps.
   We will xneed a lot of auxiliary definitions -}
